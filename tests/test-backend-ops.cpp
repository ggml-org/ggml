...
// GGML_OP_MUL + GGML_OP_ADD with a channel-wise gate
...
// GGML_OP_RMS_NORM + GGML_OP_MUL + GGML_OP_SCALE + GGML_OP_MUL + GGML_OP_ADD
struct test_rms_norm_mul_modulate : public test_case {
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        // Visit the norm weight before the fusion chain. Krea materializes two views from a
        // packed six-vector modulation result before using them as scale and shift.
...
// Krea2 projection normalization followed by NORMAL-to-NEOX deinterleave,
// multi-axis RoPE, and the layout pack consumed by flash attention.
...
// WAN VAE channel-last RMS normalization: transpose channels to ne[0],
// normalize and scale, then transpose back to the original image layout.
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    test_cases.emplace_back(new test_mul_add({65, 5, 1, 1})); // fallback path: vec4 fusion is intentionally inapplicable
