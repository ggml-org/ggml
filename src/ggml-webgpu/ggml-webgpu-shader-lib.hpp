...
// Depthwise conv2d: same type fields plus the input layout (WHCN vs CWHN).
struct ggml_webgpu_conv2d_dw_pipeline_key {
    ...
    bool operator==(const ggml_webgpu_conv2d_dw_pipeline_key & other) const {
        return weight_type == other.weight_type && input_type == other.input_type &&
               output_type == other.output_type && whcn == other.whcn;
    }
};
...
class ggml_webgpu_shader_lib {
  ...
  public:
    ...
    // Depthwise conv2d (GGML_OP_CONV_2D_DW). whcn selects the input layout: contiguous
    // WHCN vs contiguous-channels CWHN (two shader variants, matching the CPU reference).
