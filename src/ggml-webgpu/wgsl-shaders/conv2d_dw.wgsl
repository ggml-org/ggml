...
// Depthwise conv2d (GGML_OP_CONV_2D_DW). Ported from the Vulkan backend's
// conv2d_dw.comp; the WHCN and CWHN index math matches the CPU reference exactly.
// Two variants are compiled (WHCN defined or not), selected by the layout of the
// input (src1): contiguous -> WHCN, contiguous-channels -> CWHN.
//   weight (src0): [KW, KH, 1, C]
//   input  (src1): WHCN [IW,IH,C,N] contiguous, or CWHN (channels contiguous)
//   output (dst) : same layout as the input; written contiguously by flat index.
