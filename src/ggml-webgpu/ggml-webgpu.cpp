...
// Depthwise conv2d. Same param/binding layout as conv_2d; the shader differs
...
static webgpu_encoded_op ggml_webgpu_conv_2d_dw(webgpu_context & ctx,
    ...
    std::vector<uint32_t> params = {
        ...
        (uint32_t) dst->ne[2],   // channels
        (uint32_t) dst->ne[3],   // batches
        (uint32_t) dst->ne[0],  (uint32_t) dst->ne[1],   // dst_w, dst_h
        (uint32_t) src1->ne[0], (uint32_t) src1->ne[1],  // src_w, src_h
        (uint32_t) src0->ne[0], (uint32_t) src0->ne[1],  // knl_w, knl_h
