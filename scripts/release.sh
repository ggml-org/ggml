...
echo "Step 2: Updating version in CMakeLists.txt..."
if [ "$DRY_RUN" = true ]; then
...
echo "Step 5: Preparing for next development cycle..."
case $VERSION_TYPE in
    major|minor)
...
echo "Step 6: Committing development version..."
if [ "$DRY_RUN" = true ]; then
...
if [ "$DRY_RUN" = true ]; then
    ...
    echo "  • Would have released version: $NEW_VERSION"
...
if [ "$DRY_RUN" = false ]; then
    echo "Next steps:"
    echo "  • Review the commits and tag"
    echo "  • Push to remote: git push origin master && git push origin v$NEW_VERSION"
    echo ""
fi
